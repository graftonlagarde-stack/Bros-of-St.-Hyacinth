...
<p>While in the gym, Bros of St. Hyacinth will listen to readings of Holy Scripture, so that the soul may suffer purification, and exercise be made into a holy act, rather than a worldly one.</p>`,
...
<ul><li>At some time outside of the meeting, pray the Holy Rosary. Bonus points if it's in Latin.</li></ul>`,
  discipline: `<ul><li>Wake up every day at 6:15</li><li>Only take cold showers (as cold as the water will get)</li><li>Never waste time on the internet</li><li>Drink raw eggs every day (creatine recommended)</li><li>Prefer hunger to unhealthy food</li><li>Weekly confession</li><li>Daily Mass</li><li>Daily Rosary</li></ul>`,
  monastic_habit: `<ul><li>Repeal 19 Shirt</li><li>Cargo Shorts</li><li>Hawaiian Shirt</li><li>Pit Vipers</li></ul>`,
};
...
function RulePage({ user }) {
  ...
  const [renamingId, setRenamingId]     = useState(null); // id of section being renamed
  ...
  useEffect(() => {
    if (editMode) {
      sectionIndex.forEach(sec => {
        ...
        if (el) el.innerHTML = contents[sec.id] ?? "";
      });
    }
  }, [editMode]);

  // ── Persist helpers ─────────────────────────────────────────────────────────
  const apiPut = async (id, content) => {
    const token = localStorage.getItem("auth_token");
  ...
  const saveAll = async () => {
    setSaving(true);
    // Save section index
    await apiPut("__index__", JSON.stringify(sectionIndex));
    // Save each section content
    for (const sec of sectionIndex) {
      const html = editorRefs.current[sec.id]?.innerHTML ?? contents[sec.id] ?? "";
      await apiPut(sec.id, html);
      setContents(prev => ({ ...prev, [sec.id]: html }));
    }
    setSaving(false);
  };
  ...
  const removeSection = async (id) => {
    ...
    setSectionIndex(prev => prev.filter(s => s.id !== id));
    setContents(prev => { const n = { ...prev }; delete n[id]; return n; });
    // Also remove from DB
    await apiPut(id, "");
    await apiPut("__index__", JSON.stringify(sectionIndex.filter(s => s.id !== id)));
  };
  ...
  const startRename = (id, label) => { setRenamingId(id); setRenameVal(label); };
  const commitRename = () => {
    if (!renameVal.trim()) { setRenamingId(null); return; }
    setSectionIndex(prev => prev.map(s => s.id === renamingId ? { ...s, label: renameVal.trim() } : s));
    setRenamingId(null);
  };
  ...
  // ── Toolbar ─────────────────────────────────────────────────────────────────
  const TB = {
    cursor:"pointer", background:"rgba(136,255,0,0.07)", border:"1px solid rgba(136,255,0,0.2)",
    ...
    <div className="page" style={{ position:"relative" }}>
      ...
      {sectionIndex.map((sec, idx) => (
        <div key={sec.id} className="card" style={{ overflow: editMode && isArchAdmin ? "visible" : "hidden" }}>
          ...
          {/* Section header */}
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom: editMode && isArchAdmin ? 4 : 16, flexWrap:"wrap" }}>
            {/* Label — editable inline when renaming */}
            {isArchAdmin && editMode && renamingId === sec.id ? (
              <input
                value={renameVal}
                ...
                onBlur={commitRename}
                autoFocus
                style={{ flex:1, background:"rgba(0,0,0,0.4)", border:"1px solid rgba(136,255,0,0.5)",
                  color:"var(--accent)", fontFamily:"'Orbitron',sans-serif", fontSize:11, fontWeight:700,
                  letterSpacing:2, textTransform:"uppercase", padding:"4px 8px", borderRadius:3, outline:"none" }}
              />
            ) : (
              <div className="card-title" style={{ marginBottom:0, flex:1,
                cursor: isArchAdmin && editMode ? "pointer" : "default" }}
                onDoubleClick={() => isArchAdmin && editMode && startRename(sec.id, sec.label)}
                title={isArchAdmin && editMode ? "Double-click to rename" : undefined}>
                {sec.label}
                {isArchAdmin && editMode && (
                  <span style={{ marginLeft:8, fontSize:9, color:"var(--muted)", fontWeight:400, letterSpacing:1 }}>
                    dbl-click to rename
                  </span>
                )}
              </div>
            ...
            {isArchAdmin && editMode && (
              <div style={{ display:"flex", gap:4, flexShrink:0 }}>
                <button style={{...TB, padding:"2px 7px", fontSize:11}} title="Move up"
                  onMouseDown={e=>{e.preventDefault();moveSection(sec.id,-1);}}>↑</button>
                <button style={{...TB, padding:"2px 7px", fontSize:11}} title="Move down"
                  onMouseDown={e=>{e.preventDefault();moveSection(sec.id,1);}}>↓</button>
                <button style={{...TB, padding:"2px 7px", fontSize:11,
                  color:"rgba(255,80,80,0.8)", borderColor:"rgba(255,80,80,0.3)"}}
                  title="Delete section"
                  onMouseDown={e=>{e.preventDefault();removeSection(sec.id);}}>✕</button>
              </div>
            )}
          </div>
          ...
          <div
            ...
            style={{
              fontSize:13, lineHeight:1.75, color:"#f4ffe8",
            ...
            {...(!editMode || !isArchAdmin
              ? { dangerouslySetInnerHTML: { __html: contents[sec.id] ?? "" } }
              : {})}
          />
        </div>
      ))}
      ...
      <style>{`
        [contenteditable="true"], .rule-card { color:#f4ffe8 !important; }
        [contenteditable="true"] p, .rule-card p { margin:0 0 10px; color:#f4ffe8 !important; }
        [contenteditable="true"] h2, .rule-card h2 { font-size:16px; font-weight:700; color:#88ff00 !important; margin:14px 0 8px; letter-spacing:2px; text-transform:uppercase; }
        [contenteditable="true"] h3, .rule-card h3 { font-size:13px; font-weight:700; color:#f4ffe8 !important; margin:10px 0 6px; letter-spacing:1px; }
        [contenteditable="true"] ul, [contenteditable="true"] ol, .rule-card ul, .rule-card ol { margin:0 0 10px !important; padding-left:28px !important; }
        [contenteditable="true"] ul, .rule-card ul { list-style-type:disc !important; }
        [contenteditable="true"] ol, .rule-card ol { list-style-type:decimal !important; }
        [contenteditable="true"] li, .rule-card li { margin-bottom:4px !important; display:list-item !important; color:#f4ffe8 !important; }
        [contenteditable="true"] ul ul, .rule-card ul ul { list-style-type:circle !important; padding-left:22px !important; }
        [contenteditable="true"] ol ol, .rule-card ol ol { list-style-type:lower-alpha !important; padding-left:22px !important; }
        [contenteditable="true"] blockquote, .rule-card blockquote { margin:12px 0; padding:10px 16px; border-left:2px solid rgba(136,255,0,0.4); background:rgba(136,255,0,0.04); color:#f4ffe8 !important; font-style:italic; border-radius:0 3px 3px 0; }
        [contenteditable="true"] hr, .rule-card hr { border:none; border-top:1px solid rgba(136,255,0,0.2); margin:12px 0; }
        [contenteditable="true"] img, .rule-card img { max-width:100%; border-radius:4px; margin:8px 0; display:block; }
        [contenteditable="true"] strong, .rule-card strong { color:#88ff00 !important; }
        [contenteditable="true"] em, .rule-card em { color:#f4ffe8 !important; font-style:italic; }
      `}</style>
          ...
          {editMode && (
            <>
              <button className="btn btn-sm"
                style={{ fontSize:10, padding:"6px 14px", background:"rgba(136,255,0,0.08)",
                  border:"1px solid rgba(136,255,0,0.3)", color:"var(--accent)",
                  fontFamily:"'Orbitron',sans-serif", letterSpacing:1, cursor:"pointer", borderRadius:3 }}
                onClick={addSection}>
                + Add Section
              </button>
...
// Audio files are served from Cloudinary under the bible-audio/ folder.
// To add a track: upload the .mp3 to Cloudinary at bible-audio/<filename>.mp3 and add an entry here.
const PERMANENT_TRACKS = [
  { id: 1,  title: "Genesis",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Genesis.mp3" },
  { id: 2,  title: "Exodus",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Exodus.mp3" },
  { id: 3,  title: "Leviticus",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Leviticus.mp3" },
  { id: 4,  title: "Numbers",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Numbers.mp3" },
  { id: 5,  title: "Deuteronomy",           artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Deuteronomy.mp3" },
  { id: 6,  title: "Josue",                 artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Josue.mp3" },
  { id: 7,  title: "Judges",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Judges.mp3" },
  { id: 8,  title: "Ruth",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Ruth.mp3" },
  { id: 9,  title: "I Kings",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-Kings.mp3" },
  { id: 10, title: "II Kings",              artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-Kings.mp3" },
  { id: 11, title: "III Kings",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/III-Kings.mp3" },
  { id: 12, title: "IV Kings",              artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/IV-Kings.mp3" },
  { id: 13, title: "I Paralipomenon",       artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-Paralipomenon.mp3" },
  { id: 14, title: "II Paralipomenon",      artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-Paralipomenon.mp3" },
  { id: 15, title: "I Esdras",              artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-Esdras.mp3" },
  { id: 16, title: "II Esdras",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-Esdras.mp3" },
  { id: 17, title: "Tobias",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Tobias.mp3" },
  { id: 18, title: "Judith",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Judith.mp3" },
  { id: 19, title: "Esther",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Esther.mp3" },
  { id: 20, title: "Job",                   artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Job.mp3" },
  { id: 21, title: "Psalms",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Psalms.mp3" },
  { id: 22, title: "Proverbs",              artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Proverbs.mp3" },
  { id: 23, title: "Ecclesiastes",          artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Ecclesiastes.mp3" },
  { id: 24, title: "Canticle of Canticles", artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Canticle-of-Canticles.mp3" },
  { id: 25, title: "Wisdom",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Wisdom.mp3" },
  { id: 26, title: "Ecclesiasticus",        artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Ecclesiasticus.mp3" },
  { id: 27, title: "Isaias",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Isaias.mp3" },
  { id: 28, title: "Jeremias",              artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Jeremias.mp3" },
  { id: 29, title: "Lamentations",          artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Lamentations.mp3" },
  { id: 30, title: "Baruch",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Baruch.mp3" },
  { id: 31, title: "Ezekiel",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Ezekiel.mp3" },
  { id: 32, title: "Daniel",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Daniel.mp3" },
  { id: 33, title: "Osee",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Osee.mp3" },
  { id: 34, title: "Joel",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Joel.mp3" },
  { id: 35, title: "Amos",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Amos.mp3" },
  { id: 36, title: "Abdias",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Abdias.mp3" },
  { id: 37, title: "Jonas",                 artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Jonas.mp3" },
  { id: 38, title: "Micheas",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Micheas.mp3" },
  { id: 39, title: "Nahum",                 artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Nahum.mp3" },
  { id: 40, title: "Habacuc",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Habacuc.mp3" },
  { id: 41, title: "Sophonias",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Sophonias.mp3" },
  { id: 42, title: "Aggeus",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Aggeus.mp3" },
  { id: 43, title: "Zacharias",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Zacharias.mp3" },
  { id: 44, title: "Malachias",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Malachias.mp3" },
  { id: 45, title: "I Machabees",           artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-Machabees.mp3" },
  { id: 46, title: "II Machabees",          artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-Machabees.mp3" },
  { id: 47, title: "Matthew",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Matthew.mp3" },
  { id: 48, title: "Mark",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Mark.mp3" },
  { id: 49, title: "Luke",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Luke.mp3" },
  { id: 50, title: "John",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/John.mp3" },
  { id: 51, title: "Acts",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Acts.mp3" },
  { id: 52, title: "Romans",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Romans.mp3" },
  { id: 53, title: "I Corinthians",         artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-Corinthians.mp3" },
  { id: 54, title: "II Corinthians",        artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-Corinthians.mp3" },
  { id: 55, title: "Galatians",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Galatians.mp3" },
  { id: 56, title: "Ephesians",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Ephesians.mp3" },
  { id: 57, title: "Philippians",           artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Philippians.mp3" },
  { id: 58, title: "Colossians",            artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Colossians.mp3" },
  { id: 59, title: "I Thessalonians",       artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-Thessalonians.mp3" },
  { id: 60, title: "II Thessalonians",      artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-Thessalonians.mp3" },
  { id: 61, title: "I Timothy",             artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-Timothy.mp3" },
  { id: 62, title: "II Timothy",            artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-Timothy.mp3" },
  { id: 63, title: "Titus",                 artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Titus.mp3" },
  { id: 64, title: "Philemon",              artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Philemon.mp3" },
  { id: 65, title: "Hebrews",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Hebrews.mp3" },
  { id: 66, title: "James",                 artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/James.mp3" },
  { id: 67, title: "I Peter",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-Peter.mp3" },
  { id: 68, title: "II Peter",              artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-Peter.mp3" },
  { id: 69, title: "I John",                artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/I-John.mp3" },
  { id: 70, title: "II John",               artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/II-John.mp3" },
  { id: 71, title: "III John",              artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/III-John.mp3" },
  { id: 72, title: "Jude",                  artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Jude.mp3" },
  { id: 73, title: "Apocalypse",            artist: "Holy Bible", src: "https://res.cloudinary.com/djoqczzyj/video/upload/bible-audio/Apocalypse.mp3" },
];
