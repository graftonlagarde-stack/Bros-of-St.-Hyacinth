...
function WorkoutPage({ username }) {
  ...
  // (visiting the page creates a session row even with no sets, so we exclude empty sessions)
...
function AvatarUpload({ user, onUpdate, uploadFn, removeFn, responseKey = "avatarUrl" }) {
  ...
  const [imgSize,   setImgSize]   = useState({ w: 0, h: 0 }); // natural dimensions once loaded
  const fileRef   = useRef(null);
  const canvasRef = useRef(null);
  ...
  const onPointerDown = (e) => {
    ...
    dragRef.current = { startX: p.clientX, startY: p.clientY, ox: offset.x, oy: offset.y };
  };
  const onPointerMove = (e) => {
    if (!dragRef.current) return;
    const p = e.touches ? e.touches[0] : e;
    setOffset({ x: dragRef.current.ox + p.clientX - dragRef.current.startX, y: dragRef.current.oy + p.clientY - dragRef.current.startY });
  };
  ...
  if (cropSrc) {
    ...
    return (
      <div style={{marginBottom:20}}>
        ...
        >
          <img ref={imgRef} src={cropSrc} onLoad={onImgLoad}
            style={{
              position:"absolute",
        ...
        <div style={{display:"flex",gap:8,marginTop:12}}>
          <button className="btn btn-primary" onClick={confirmCrop} style={{fontSize:12,padding:"6px 18px"}}>Save Photo</button>
          <button className="btn" onClick={() => setCropSrc(null)} style={{fontSize:12,padding:"6px 14px"}}>Cancel</button>
        </div>
...
function MeetPage({ currentUser, onCallActive }) {
  ...
  const switchView = useCallback((next, extraWork) => {
    setOpacity(0);
    setTimeout(() => {
      extraWork?.();
      setView(next);
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        // If an instance exists and we already joined it, reuse it
        if (window.__dailyCallInstance) {
          const state = window.__dailyCallInstance.meetingState();
          if (state === 'joined-meeting' || state === 'joining-meeting') {
            callRef.current = window.__dailyCallInstance;
            didJoin = true;
            return; // already in call, don't create a new one
          }
          try { window.__dailyCallInstance.destroy(); } catch(e) {}
    ...
    return () => {
      destroyed = true;
      // Only destroy if we actually joined — prevents StrictMode double-mount
      // from destroying an instance that's still initializing
      if (didJoin && callRef.current) {
      ...
      <div style={{position:"absolute",bottom:90,right:16,width:72,height:72,borderRadius:"50%",overflow:"hidden",
        ...
        {camOn && localStream
          ...
          : currentUser.avatarUrl
            ? <img src={currentUser.avatarUrl} alt={currentUser.displayName} crossOrigin="anonymous" style={{width:"100%",height:"100%",objectFit:"cover"}} />
...
export default function App() {
  ...
  return (
    <>
      ...
      {currentTrack && <PlayerBar track={currentTrack} isPlaying={isPlaying} setIsPlaying={setIsPlaying} tracks={PERMANENT_TRACKS} setTrack={setCurrentTrack} navExpanded={navExpanded} />}
    </>
  );
}
